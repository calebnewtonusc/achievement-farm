...
for i in $(seq $START $TARGET); do
  ...
  git checkout -b "$BRANCH" 2>/dev/null || { echo "[$i] branch create failed, skipping"; continue; }
  echo "PR $i — $(date)" >> shark.md
  git add shark.md
  git commit -m "feat: shark $i" --quiet || { echo "[$i] commit failed, skipping"; git checkout main 2>/dev/null; continue; }
  ...
  if [ -z "$PR_NUM" ]; then
    echo "[$i] PR create failed, sleeping 10s and retrying..."
    sleep 10
    PR_NUM=$(gh pr create --repo "$FULL_REPO" --head "$BRANCH" --base main --title "shark: pr $i" --body "Pull Shark $i" 2>/dev/null | grep -oE '[0-9]+$')
  fi
  if [ -z "$PR_NUM" ]; then
    echo "[$i] PR create failed again, skipping"
    git checkout main 2>/dev/null || true
    continue
  fi
  gh pr merge "$PR_NUM" --repo "$FULL_REPO" --squash 2>/dev/null || { sleep 5; gh pr merge "$PR_NUM" --repo "$FULL_REPO" --squash 2>/dev/null || true; }
