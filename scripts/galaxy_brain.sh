...
# Usage: bash scripts/galaxy_brain.sh YOUR_USERNAME YOUR_REPO [START]
#   START is optional — use it to resume if rate limited (e.g. bash galaxy_brain.sh user repo 10)
set -e
USERNAME=${1:?Usage: bash galaxy_brain.sh USERNAME REPO [START]}
REPO=${2:?Usage: bash galaxy_brain.sh USERNAME REPO [START]}
START=${3:-1}
TARGET=32
